...
const phase2FeatureRows = [
  ...
  ["机器人运维", "新增", "直播画面卡片字段", "卡片展示机器人ID、名称、版本、直播模式、开播状态、画面来源、在线状态、联网、麦克风、扬声器、电机、推流、当前告警、直播时长、当前脚本和当前阶段；桌面端采用左侧直播画面、右侧直播信息的布局，窄屏自动改为上下布局；未开播时使用明确的空闲值。", "上一版本没有面向值守场景的设备运行信息卡片。", true],
  ["机器人运维", "新增", "画面/列表/全屏视图", "三种视图使用同一筛选结果，按在线、异常、离线排序；切换视图或进入、退出全屏时保留筛选条件。", "上一版本没有面向多设备的画面墙、列表和全屏值守能力。", true],
  ["机器人运维", "新增", "告警列表字段与排序", "告警展示机器人ID、名称、版本、异常情况、持续时长和发生时间，按发生时间倒序；名称过长单行省略，异常情况完整换行。", "上一版本没有可供值守人员持续查看的设备告警列表。", true],
  ["机器人运维", "新增", "告警点击联动", "点击任一告警直接打开对应设备详情，不改变当前设备范围、开播状态、告警时间和视图。", "上一版本告警与设备运行详情缺少直接联动。", true],
  ["机器人运维", "新增", "设备详情与最近告警", "详情展示机器人基础信息、直播运行状态、设备部件状态、最后上报时间和最近6条告警；最近告警独立于页面告警类型和时间筛选。", "上一版本设备信息停留在配置字段，缺少完整运行状态和告警上下文。", true],
