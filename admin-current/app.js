...
const phase2FeatureRows = [
  ...
  ["机器人运维", "新增", "核心指标与统计口径", "展示设备总数、在线、离线、直播中、录播中、版本分布和异常告警；设备类指标按当前渠道统计，异常告警按告警类型与时间范围统计，指标卡仅展示不可点击。", "上一版本没有统一的运维总览及指标口径。", true],
  ["机器人运维", "新增", "直播画面卡片字段", "卡片展示机器人ID、名称、版本、直播模式、开播状态、画面来源、在线状态、当前告警、直播时长、当前脚本和当前阶段；卡片底部以纯文字展示网络、推流、音频和电机的正常、异常、离线或未检测状态；桌面宽屏画面区每行5个卡片，左侧直播画面保持3:2，右侧展示直播信息，较窄窗口按3列、2列、1列响应式收缩；未开播时使用明确的空闲值。", "上一版本没有面向值守场景的设备运行信息卡片。", true],
  ["机器人运维", "新增", "画面/列表/全屏视图", "三种视图使用同一筛选结果，按在线、异常、离线排序；大屏采用左侧画面区、右侧告警列表，MacBook等中等屏幕采用顶部指标、中部可滚动画面区、底部固定高度告警区，平板及更窄屏幕改为自然纵向流；桌面宽屏画面区每行展示5个卡片，直播画面保持3:2，较窄窗口按3列、2列、1列收缩；切换视图或进入、退出全屏时保留筛选条件。", "上一版本没有面向多设备的画面墙、列表和全屏值守能力。", true],
  ...
  ["机器人运维", "新增", "设备详情与最近告警", "详情ID显示为RBT加数字序号；字段按基础信息、直播运行、脚本执行、设备连接四组紧凑展示，减少无层级平铺和空间浪费；电机状态区独立展示1至15号圆形编号，正常为绿色、异常为红色、未检测为灰色；最近告警固定展示最近6条，不继承页面告警筛选。", "上一版本设备信息停留在配置字段，缺少分组后的完整运行状态、电机明细和告警上下文。", true],
  ["机器人运维", "新增", "跨页处理入口", "设备详情支持进入当前机器人的直播数据大屏和脚本执行管理；直播中进入实时数据，未开播时展示历史记录或空数据，脚本入口只定位当前机器人。", "上一版本运维、直播控制和脚本执行入口相互割裂。", true],
  ["机器人运维", "新增", "实时订阅与手动刷新", "页面进入后建立实时订阅，设备、画面、告警和直播运行数据随服务端事件持续更新；不提供自动刷新开关，也不使用定时轮询。立即刷新作为人工兜底，保留当前筛选、视图和全屏状态，失败时保留上一轮数据并提示。", "上一版本没有明确实时更新、人工兜底和失败保留规则。", true],
...
const phase2ComparisonRows = [
  ...
  ["机器人运维", "上一版本以机器人资料、技能、表演和脚本绑定为主。", "本版本补齐直播设备范围、筛选与指标口径、画面/列表/全屏值守、告警联动、设备详情、跨页处理和刷新规则。"],
...
const phase2FocusRows = [
  ...
  ["机器人运维联动", "把直播设备范围、筛选、指标、画面、告警、详情、跨页处理和刷新规则串成完整值守闭环。"],
